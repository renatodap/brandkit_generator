...
/**
 * POST /api/businesses/[id]/access-requests/[requestId]/reject
 * Reject an access request (owner/admin only)
 */
export async function POST(
  ...
  try {
    ...
    logger.error('Failed to reject access request', error as Error, { requestId: params.requestId, businessId: params.id });
    return NextResponse.json(
      {
        error: 'Failed to reject access request. Please try again.',
      },
      { status: 500 }
    );
  }
